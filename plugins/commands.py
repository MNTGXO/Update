...
logger = logging.getLogger("OTTBot.commands")
...
async def platforms(client: Client, message: Message):
    logger.info(f"/platforms from chat={message.chat.id}")
...
async def autosend_cmd(client: Client, message: Message):
    logger.info(f"/autosend from chat={message.chat.id} text={message.text}")
